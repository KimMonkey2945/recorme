...
/**
 * 미션 판정 규칙 타입. V16 {@code chk_missions_rule_type} CHECK 와 동일 집합(★ 감정 규칙 없음).
 *
 * <p>각 타입은 진행률 산출 시 {@code user_progress}(또는 레벨)의 정확히 한 컬럼만 본다 — O(1).
 * <ul>
 ...
 *   <li>{@code LEVEL}              → user_character_state.level</li>
 * </ul>
 * 임계값 키는 타입마다 다르지만(count/days/seq/level), 매퍼가 {@code threshold} 하나로 정규화해 내려준다.
 */
