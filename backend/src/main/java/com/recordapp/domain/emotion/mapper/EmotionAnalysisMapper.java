 ...
 * 멱등·낙관적 UPDATE 라 기록 CRUD 와 관심사가 다르다. ⚠️ 모든 UPDATE 는 {@code updated_at} 을
...
public interface EmotionAnalysisMapper {
	...
	/**
	 * 분석 대상 기록 스냅샷 조회. {@code PENDING} 이고 활성(미삭제)인 행만 반환한다.
	...
	/**
	 * 폴러 백스톱: 미처리(PENDING) 활성 기록 ID 를 id 오름차순으로 최대 limit 건 잠금 조회한다.
