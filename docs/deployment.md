...
- ⏳ **감정 분석 코멘트 누락**: 이모지(감정)는 나오는데 **코멘트 텍스트가 안 옴**. 응답은 오지만 코멘트
  파트가 비는 현상 → 원인 조사 필요(응답 파싱 / `max-tokens`(400) 절단 / Gemini 간헐 503 폴백 여부).
- ⏳ **로딩 러닝 영상 2회 재생**: 감정 분석 대기 로딩 영상이 **두 번** 나옴 → 한 번만 나오게 수정.
- ⏳ **작심삼일 수정 기능 추가**: 작성한 작심삼일(결심)을 **수정**하는 기능 필요.
