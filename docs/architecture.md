...
2. 🐵 **내 캐릭터** — 기록을 확정하면 **내 캐릭터가 반응**하고, 쌓일수록 **코인·미션 해금으로 꾸며진다**(원숭이 / 레서판다 2종. ⚠️ 레벨·경험치는 보상 재설계로 폐기). 캐릭터 홈이 앱의 메인이며 연출의 주인공이다 *(Phase 7 — 선택·옷장·구매·코인 적립·리액션·월간 회고까지 구현. 미션 판정·아이템 해금 지급은 범위 밖. §3.3)*
3. 🎨 **감정 메타데이터** — 감정은 **사용자가 직접 입력**(프리셋 6종 또는 자유 텍스트)하며, 달력 점 색·감정 칩·회고 통계에만 쓰인다 *(✅ 구현 완료 — Task 024·025. LLM 자동 분석은 `record.analysis.enabled` 기본 false 로 꺼져 있다)*
...
| **감정 LLM 분석 비활성화 + 수동 입력 전환** | ✅ | Task 024(백엔드, V19)·025(앱). `record.analysis.enabled` 기본 **false** — 확정 시 즉시 `DONE`이고 감정은 사용자가 직접 입력한다(프리셋 6종 또는 자유 텍스트 ≤20자, 상호 배타). 감정 시각 연출은 전부 제거됐고 달력 점 색·감정 칩·회고 통계에만 쓰인다 |
...
| **탭 재편 + 캐릭터 홈** | ✅ | Task 029. 탭은 `[캐릭터 홈(/)][캘린더][작심삼일][친구][프로필]` 5개이고 캐릭터 홈이 index 0(로그인 후 첫 화면). 목록은 캘린더 앱바로, 프로필은 탭으로 승격. ⚠️ 별도 상점·미션 화면은 보상 재설계로 폐기(구매는 옷장 통합). FCM 딥링크는 경로 문자열 push라 인덱스 변경과 무관 |
| **Rive `.riv` 아트보드 전환** | ⏳ **미제작** | Task 031. `rive` 패키지 미도입 |
| **친구 둘러보기(읽기 전용)** | ✅ | 친구 목록에서 이름 탭 → `/friends/browse/:userUuid`(상단 탭 3개: 홈·캘린더·작심삼일). 백엔드 `FriendBrowseService`(권한 게이트 `resolveFriendId` 단일 지점, 비친구 전부 404 은닉) + `FriendBrowseController`(GET 4개). 캘린더는 `visibility IN ('FRIENDS','PUBLIC')`만 노출하고 캐릭터 응답에서 코인·보상 제외. 하단 탭 index 3이 피드→친구로 교체되고 피드는 셸 밖 push |
