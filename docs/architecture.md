...
2. 🐵 **내 캐릭터** — 기록을 확정하면 **내 캐릭터가 반응**하고, 쌓일수록 레벨·코인·미션으로 **꾸며진다**(원숭이 / 레서판다 2종). 캐릭터 홈이 앱의 메인이며 연출의 주인공이다 *(Phase 7 진행 중 — 현재 캐릭터 선택·옷장 착용·미션 조회까지 구현, 보상·홈은 미구현. §3.3)*
...
| **탭 재편 + 캐릭터 홈·옷장·상점·미션 UI** | ⏳ **미구현** | Task 029 본편·030. FCM 딥링크 회귀 위험으로 분리 |
...
- **캐릭터 PNG 배경 불투명**: 현재 `assets/characters/*.png`는 **배경이 크림색으로 채워져 있다**(투명 PNG 교체 예정). `CharacterStage`가 크림톤 `AppColors.paper` 카드 위에 올려 흰 박스가 튀지 않게 흡수하고 있으나, **아이템 오버레이(Task 030)에는 투명 PNG가 사실상 전제**다.
