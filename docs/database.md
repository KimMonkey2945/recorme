...
- 스키마 변경은 MyBatis와 무관하게 Flyway로 버전 관리.
- 초기 스키마: `backend/src/main/resources/db/migration/V1__init.sql` (위 DDL).
- 마스터 데이터(`emotion_types`, 초기 `themes`/`tracks`)는 `V2__seed_master.sql` 등으로 분리.
- 운영 환경에서는 `flyway.clean` 비활성화, 모든 변경은 신규 버전 마이그레이션으로만.
