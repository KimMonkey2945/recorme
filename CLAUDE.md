...
### 옷장 카탈로그 (2026-07-16, V21 — 중요)

옷장 아이템은 **부위별 착용 5종**으로 확정됐다(그 외 전부 제거). **전부 `acquire_type=COIN`(구매 대상)이라 기본 미보유(잠금)**이며, 구매 기능(Task 028 잔여) 구현 전까지는 가격만 노출되고 착용 불가다. DEFAULT/MISSION 아이템은 없어 신규 유저는 빈 옷장으로 시작한다(캐릭터 본체는 통짜 PNG라 정상 렌더).

| 슬롯 | code | 이름 | 코인 |
|---|---|---|---:|
| HAT | `HAT_CAP_BLACK` | 누구나 소화할 수 있는 검은색 캡모자 | 15 |
| GLASSES | `GLASSES_ROUND` | 안경알은 없지만 멋짐을 위한 검은색 뿔테안경 | 15 |
| OUTFIT | `OUTFIT_LOVE_HOOD` | 사랑하는 사람에게 보여주고 싶은 낭낭한 후드티 | 50 |
| BOTTOM | `BOTTOM_CARGO_SAND` | 입으면 사막에서도 살아남을 것 같은 바지 | 50 |
| SHOES | `SHOES_MAX95` | 신발에 에어가 없으면 허리가 아픈 사람을 위한 에어빵빵 신발 | 20 |

- **V21** `V21__replace_item_catalog.sql`: 미션 아이템 보상 NULL 정리 → 슬롯 CHECK에 **BOTTOM·SHOES 추가** → V15 시드 5종 삭제(CASCADE) → 신규 5종 COIN + character_items 10행. 백엔드 `ItemSlot` enum도 BOTTOM·SHOES 확장(미갱신 시 MyBatis slot 매핑 실패).
- 앱: Fake 카탈로그 5종 COIN·미보유, 옷장 슬롯 탭 5개(HAT/GLASSES/OUTFIT/BOTTOM/SHOES)로 축소, `wardrobe_test` 소유 주입(`FakeCharacterRepository(ownedGroups: {...})`)로 착용 흐름 검증. 캐릭터별 렌더 에셋(`app/assets/items/*_{monkey,red_panda}.png`)은 이미 완비.
- ✅ **상점 구매 구현(2026-07-16)**: `POST /characters/items/{groupCode}/purchase`(`WardrobeController`→`CharacterRewardService.purchase` — 게이트→조건부 차감→소유, `COIN_INSUFFICIENT`/`FEATURE_DISABLED`, `coin-enabled` 기본 **on**). 앱은 옷장 잠금 시트의 **구매 버튼**(`locked_item_sheet` + `PurchaseController`)으로 산다. 이제 "코인 벌기(028)→사기(구매)→입기(옷장)" 루프 완성. 백엔드 전체 그린·앱 141개 통과.
...
- **남은 Task**:
  ...
  - **028 — 보상 엔진**: ✅ **부분 완료(2026-07-16, V20)** — **코인 적립만** 구현. `CharacterRewardService`(멱등 게이트→코인→진척→마일스톤→대사→payload, `REQUIRES_NEW`) + `CharacterEventListener`(AFTER_COMMIT·`@Async("characterExecutor")`) + `CharacterRewardBackfillPoller`(백스톱) + `CharacterRewardController`(`GET /me/wallet`·`/me/rewards`·`POST /me/rewards/ack`·`GET /me/reaction`·`POST /me/attendance`). 단방향 결합은 `global/event/{DiaryConfirmedEvent,ResolutionProgressEvent}`(diary·resolution 은 character 를 import 안 함 — 정적 테스트로 강제). 적립 트리거는 `docs/coin-rewards.md` 표 그대로: **출석·기록 확정·작심삼일 1·2일차·완주·연속 7/30/60 마일스톤**. 값·마일스톤은 `record.character.coin.*` 설정으로 **코드 변경 없이 조정**(`CharacterCoinProperties`, `streak` Map). V20 은 `character_events.event_type` 고정 CHECK 제거(트리거 확장 유연성 — 무결성은 event_key UNIQUE·balance CHECK 유지). 테스트 14개 통과. **상점 구매(코인 소비)도 이후 구현됨**(`CharacterRewardService.purchase`·`POST /characters/items/{groupCode}/purchase`·`COIN_INSUFFICIENT`/`FEATURE_DISABLED`·`coin-enabled` 기본 on). ⚠️ **여전히 범위 밖**: 미션 판정·아이템 해금 지급(미션은 조회만, 연속 7일은 설정 마일스톤 `streak.7`로 지급). 경험치/레벨은 재설계로 제거됐으니 만들지 말 것.
